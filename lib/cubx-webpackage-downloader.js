(function () {
  ...
  var WebpackageDownloader = function () {
  ...
  WebpackageDownloader.prototype._downloadWebpackage = async function (webpackageId) {
    var artifactsQeue = [];
    try {
      var result = await this._fetchManifest(webpackageId);
      var manifest = result.data;
      this._saveManifest(manifest, webpackageId);
      if (manifest.hasOwnProperty('artifacts')) {
        var artifacts = result.data.artifacts;
        var artifactTypes = ['apps', 'compoundComponents', 'elementaryComponents', 'utilities'];
  ...
  WebpackageDownloader.prototype._downloadArtifact = async function (artifact, webpackageId) {
    var artifactSourceUrl = this.baseUrl + webpackageId + '/' + artifact.artifactId;
    var artifactTargetUrl = path.join(this.targetDirectory, webpackageId, artifact.artifactId);
    var artifactFilesQeue = [];
  ...
  WebpackageDownloader.prototype._downloadResource = async function (artifactSourceUrl, artifactTargetPath, resource) {
    ...
    if (typeof resource === 'string') {
      filesToDownload.push(await this._downloadFile(artifactSourceUrl, artifactTargetPath, resource));
    } else if (typeof resource === 'object') {
      if (resource.hasOwnProperty('dev')) {
        filesToDownload.push(await this._downloadFile(artifactSourceUrl, artifactTargetPath, resource.dev));
      }
      if (resource.hasOwnProperty('prod')) {
        filesToDownload.push(await this._downloadFile(artifactSourceUrl, artifactTargetPath, resource.prod));
      }
    }
  ...
  WebpackageDownloader.prototype._downloadRunnable = async function (artifactSourceUrl, artifactTargetPath, runnable) {
    return this._downloadFile(artifactSourceUrl, artifactTargetPath, runnable.path);
  };

  WebpackageDownloader.prototype._downloadFile = async function (artifactSourceUrl, artifactTargetPath, relativePath) {
    var sourceUrl = artifactSourceUrl + '/' + relativePath;
    var targetPath = artifactTargetPath + '/' + relativePath;
    try {
      var response = await this._axios.request({url: sourceUrl, responseType: 'text'});
  ...
  WebpackageDownloader.prototype._saveFile = function (fileContent, targetPath) {
    return new Promise(function (resolve, reject) {
      fs.ensureDir(path.dirname(targetPath), function () {
        fs.writeFile(targetPath, fileContent, 'utf8', function (error, data) {
          if (error) {
  ...
  WebpackageDownloader.prototype._isValidListOfWebpackages = function (listOfWebpackages) {
    var webpackageIdRegExp = /^([a-z0-9]+||([a-z0-9]+[a-z0-9-][a-z0-9]+)*)(\.([a-z0-9]+||([a-z0-9]+[a-z0-9-][a-z0-9]+)*))*[@](\d+)(\.[\d]+)*(-SNAPSHOT)?$/;
