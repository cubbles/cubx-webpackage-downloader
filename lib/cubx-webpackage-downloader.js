(function () {
  ...
  WebpackageDownloader.prototype.downloadWebpackages = async function (listOfWebpackagesId, outputDirectory, baseUrl) {
    if (isValidPath(outputDirectory)) {
      this.outputDirectory = outputDirectory;
    } else {
      console.error(this._messagePrefix, 'Invalid \'outputDirectory\' path. Given:', outputDirectory);
      throw new Error('Invalid \'outputDirectory\'');
    }
    if (validUrl.isWebUri(baseUrl)) {
      this.baseUrl = baseUrl.endsWith('/') ? baseUrl : baseUrl + '/';
    } else {
      console.error(this._messagePrefix, 'Invalid \'baseUrl\' url. Given:', baseUrl);
    ...
    if (this._isValidListOfWebpackages(listOfWebpackagesId)) {
      ...
      console.error(this._messagePrefix, 'Invalid \'listOfWebpackages\'. It should contain valid webpackage Ids, ' +
        'e.g., [ \'webpackage1@1.0.0\' ]. Given:', listOfWebpackagesId);
  ...
  WebpackageDownloader.prototype._downloadArtifact = async function (artifact, webpackageId) {
    ...
    let artifactTargetUrl = path.join(this.outputDirectory, webpackageId, artifact.artifactId);
  ...
  WebpackageDownloader.prototype._saveManifest = function (manifest, webpackageId) {
    return this._saveFile(manifest, path.join(this.outputDirectory, webpackageId, 'manifest.webpackage'));
  };
